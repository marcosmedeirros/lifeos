        ...
        <div class="main-shell">
            <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                ...
                <div class="flex gap-3 items-center">
                    <div class="glass-card px-6 py-3 rounded-xl border border-purple-500/30 bg-purple-500/10">
                        <span class="text-sm font-medium text-slate-300 mr-2">Seu XP:</span>
                        <span class="text-2xl font-bold text-purple-400" id="user-xp-display">0 XP</span>
                    </div>
                    <button onclick="openModal('modal-game-task', false); openMissionModal(0);" class="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2 rounded-lg font-bold shadow-lg transition">
                        <i class="fas fa-plus mr-1"></i> Missão
                    </button>
                    <button onclick="openModal('modal-game-reward', false); openRewardModal(0);" class="bg-purple-600 hover:bg-purple-500 text-white px-5 py-2 rounded-lg font-bold shadow-lg transition">
            ...
            <div class="mt-8 glass-card p-6 rounded-2xl shadow-2xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <h4 class="text-lg font-bold text-purple-400 mb-4">Suas Recompensas</h4>
...
<div id="modal-overlay" class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm hidden z-50 flex items-center justify-center p-4" onclick="closeModal()">
    <div id="modal-content" class="modal-glass rounded-2xl p-8 w-full max-w-md relative max-h-[90vh] overflow-y-auto" onclick="event.stopPropagation()">
        ...
        <h3 class="text-2xl font-bold mb-6 text-emerald-400" id="task-modal-title">Nova Missão</h3>
        ...
        <div class="space-y-5">
            ...
            <div class="flex gap-3 pt-4">
                <button type="submit" class="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl shadow-lg transition">Salvar Missão</button>
    ...
    <form id="modal-game-reward" class="modal-form hidden" onsubmit="submitGameReward(event)">
        <h3 class="text-2xl font-bold mb-6 text-purple-400" id="reward-modal-title">Nova Recompensa</h3>
        ...
        <div class="space-y-5">
            ...
            <div class="flex gap-3 pt-4">
                <button type="submit" class="flex-1 bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 rounded-xl shadow-lg transition">Salvar Recompensa</button>
...
function renderAdminLists() {
    ...
    const rewardHtml = rewards.map(r => `
        <div onclick="openRewardModal(${r.id})" class="flex justify-between items-center bg-slate-800 p-3 rounded-lg border-l-4 mb-2 border-${r.color}-500 hover:bg-slate-700 transition cursor-pointer">
            <span class="font-medium">${r.name} <span class="text-purple-400 text-xs font-bold">(${r.cost} XP)</span></span>
            <i class="fas fa-pen text-slate-400 text-sm"></i>
        </div>
    `).join('');
...
function renderGame() {
    ...
    const rewardsHtml = rewards.map(reward => {
        ...
        return `
            <div class="flex items-center p-3 bg-slate-800/50 rounded-xl border border-slate-700/50 hover:bg-slate-800 transition">
                <div class="w-10 h-10 rounded-full flex items-center justify-center bg-purple-500/20 text-purple-400 mr-4">
                    <i class="fas ${reward.icon}"></i>
                </div>
                <div class="flex-1">
                    <p class="font-bold text-white">${reward.name}</p>
                    <span class="text-sm text-purple-400 font-mono">${reward.cost} XP</span>
                </div>
                <button 
